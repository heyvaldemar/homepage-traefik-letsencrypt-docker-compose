#!/usr/bin/env bash
# homepage-restore-config.sh [backup-file-name]
#
# Replaces Homepage's application data with one of the archives the backups
# service wrote.
#
#   ./homepage-restore-config.sh               list and ask
#   ./homepage-restore-config.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than homepage.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-homepage}"
APP_SERVICE="homepage"
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Homepage." >&2
  exit 1
fi
