#!/bin/bash
# End-to-end tests for the homepage-traefik-letsencrypt-docker-compose
# backup + restore flow.
#
# Homepage has no database at all: everything it knows is the YAML you wrote in
# the config directory. That makes the archive the whole application state, and
# the test that matters is whether a file written before a backup comes back
# after a restore that has since destroyed it.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-homepage}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-homepage-traefik-letsencrypt-docker-compose.yml}"
...
: "${DATA_BACKUPS_PATH:=/srv/homepage-config/backups}"
: "${DATA_BACKUP_NAME:=homepage-config-backup}"
: "${DATA_PATH:=/config}"
: "${BACKUP_INTERVAL:=24h}"
...
APP_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq homepage | head -n 1)"
[[ -n "$BACKUPS_CONTAINER" ]] || { echo "error: backups container not found" >&2; exit 1; }
[[ -n "$APP_CONTAINER" ]] || { echo "error: homepage container not found" >&2; exit 1; }
...
test_env_required() {
  grep -qE '^HOMEPAGE_HOSTNAME=.' .env || { echo "  HOMEPAGE_HOSTNAME is not set in .env" >&2; return 1; }
...
test_backup_carries_the_service_definitions() {
  # A tarball of /config that happens to contain a directory proves very
  # little. services.yaml and settings.yaml ARE the dashboard: hand-written,
  # never generated, and the only thing here that cannot be recreated by
  # starting the container again.
  local f listing
  f=$(list_backups | tail -1)
  listing=$(bk "tar -tzf '$f'")
  grep -q '^config/services.yaml' <<<"$listing" \
    || { echo "  $f does not carry services.yaml" >&2; return 1; }
  grep -q '^config/settings.yaml' <<<"$listing" \
    || { echo "  $f does not carry settings.yaml" >&2; return 1; }
  echo "  the archive carries the service definitions and the settings"
}
...
test_restore_roundtrip() {
  ...
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./homepage-restore-config.sh "$(basename "$f")" > /dev/null \
    || { echo "  ./homepage-restore-config.sh failed" >&2; return 1; }
...
run_test test_backup_carries_the_service_definitions
